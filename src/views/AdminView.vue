    ...
    <div v-else class="admin-panel">
      <div class="admin-header">
        ...
        <div class="header-actions">
          <button @click="goHome" class="btn btn-secondary btn-small">
            🏠 Accueil
          </button>
          <button @click="logout" class="btn btn-danger btn-small">
            🚪 Déconnexion
          </button>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'themes'" class="tab-content">
        ...
        <div class="section">
          <h3 class="section-title">
            📋 Thèmes existants ({{ quizStore.themes.length }})
          </h3>
        ...
        <div class="section">
          ...
          <div class="form-group">
            ...
            >
              ...
              <span
                v-if="questionForm.correct === index"
                class="status-badge correct"
              >
                ✓ Correcte
              </span>
              <span v-else class="status-badge incorrect">
                ✗ Incorrecte
              </span>
          ...
          <div class="form-group">
            ...
            <select v-model="questionForm.correct" class="correct-select">
              <option
                v-for="(answer, index) in questionForm.answers"
                :key="index"
                :value="index"
              >
                Réponse {{ index + 1 }}: {{ answer || '(vide)' }}
              </option>
            </select>
          </div>
          ...
          <div class="form-row">
            <div class="form-group" style="width: 150px;">
              ...
              <input
                v-model.number="questionForm.time"
                type="number"
                min="5"
                max="60"
              />
        ...
        <div class="section">
          ...
          >
            <div class="theme-header" @click="toggleTheme(theme.id)">
              ...
              <span
                class="chevron"
                :class="{ open: expandedThemes.includes(theme.id) }"
              >
                ▼
              </span>
            </div>
            ...
            >
              ...
              <div
                v-for="(q, qIndex) in theme.questions"
                :key="qIndex"
                class="question-item"
              >
                <div class="question-content">
                  <div class="question-text-admin">
                    {{ qIndex + 1 }}. {{ q.question }}
                  </div>
                  <div class="question-answers">
                    ...
                    >
                      {{ a }}
                      <span v-if="aIndex === q.correct" class="chip-status">✓</span>
                    </span>
                  </div>
...
/* === RÉPONSES === */
.answer-row-admin { display: flex; gap: 10px; align-items: center; margin-bottom: 15px; padding: 10px; background: white; border-radius: 10px; border: 2px solid #e5e7eb; }
.answer-input { flex: 1; }
.status-badge { padding: 4px 12px; border-radius: 20px; font-size: 0.85em; font-weight: 600; white-space: nowrap; }
.status-badge.correct { background: #10b981; color: white; }
.status-badge.incorrect { background: #e5e7eb; color: #64748b; }
.correct-select { background: #f0fdf4; border-color: #10b981 !important; }
...
.answer-chip { padding: 6px 14px; background: #e5e7eb; border-radius: 20px; font-size: 0.85em; color: #64748b; display: flex; align-items: center; gap: 6px; }
.answer-chip.correct { background: #10b981; color: white; font-weight: 600; }
...
@media (max-width: 768px) {
  ...
  .answer-row-admin { flex-direction: column; align-items: stretch; }
}
</style>
